...
public class ChainerTest {
  ...
  public void testChainingTwoOperations(TestContext context) {
    ...
    Future<Integer> chain = Chainer.chain(1, this::opReturningFuture, this::opReturningFuture);
  ...
  public void testChainingThreeOperations(TestContext context) {
    Async async = context.async();
    Future<Integer> chain = Chainer
        .chain(1, this::opReturningFuture, this::opReturningFuture, this::opReturningFuture);
    chain.setHandler(result -> {
      if (result.failed()) {
        context.fail(result.cause());
      } else {
  ...
  public void testChainingThreeOperationsWithFutureAsInput(TestContext context) {
    ...
    Future<Integer> chain = Chainer.chain(future,
  ...
  public void testChainingThreeOperationsReturningFuturesWithFailure(TestContext context) {
    Async async = context.async();
    Future<Integer> chain = Chainer
        .chain(1, this::opReturningFuture, this::opReturningAFailedFuture, this::opReturningFuture);
    chain.setHandler(result -> {
      if (result.failed()) {
        context.assertTrue(result.cause().getMessage().contains("D'oh"));
        async.complete();
  ...
  @Test
  public void testChainingThreeOperationsReturningFuturesWithFailureInterruption(
      TestContext context) {
    Async async = context.async();
    Future<Integer> chain = Chainer
        .chain(1, this::opReturningFuture, this::opReturningAFailedFuture, this::opNotExecuted);
    chain.setHandler(result -> {
      if (result.failed()) {
        context.assertFalse(spyNotExecuted.get());
        context.assertTrue(result.cause().getMessage().contains("D'oh"));
  ...
  public void chainingWithDifferentTypes(TestContext context) {
    Async async = context.async();
    Future<Buffer> chain = Chainer.chain(1,
        input -> {
